...
class HW2_sql():
    ...
    def part_1_a_i(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_1_a_i_sql = "create table if not exists movies( id integer PRIMARY KEY, title text, score real);"
        ######################################################################
    ...
    def part_1_b_movies(self,connection,path):
        ...
        with open('./data/movies.csv', newline='') as csvfile:
            ...
            for row in spamreader:
                id, title, score = row
                connection.execute("INSERT INTO movies VALUES (?,?,?)",(int(id),title,score))
                connection.commit()
       ######################################################################
    ...
    def part_1_b_movie_cast(self,connection, path):
        ############### CREATE IMPORT CODE BELOW ############################
        with open('./data/movie_cast.csv', newline='') as csvfile:
            spamreader = csv.reader(csvfile)
            
            for row in spamreader:
                movie_id, cast_id, cast_name, birthday, popularity = row
                connection.execute("INSERT INTO movie_cast VALUES (?,?,?,?,?)",(int(movie_id),int(cast_id),cast_name,birthday,popularity))
                connection.commit()
        ######################################################################
    ...
    def part_1_c(self,connection):
        ...
        ############### CREATE IMPORT CODE BELOW ############################
        part_1_c_insert_sql = "INSERT INTO cast_bio(cast_id,cast_name,birthday,popularity) SELECT er.cast_id, er.cast_name, er.birthday, er.popularity FROM movie_cast er GROUP BY cast_id;"
        ######################################################################
    ...
    def part_3(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_3_sql = "WITH total_movies AS (SELECT COUNT(*) AS count FROM movies), movies_in_range AS (SELECT COUNT(*) AS count FROM movies WHERE score BETWEEN 7 AND 20) SELECT printf('%.2f',(movies_in_range.count * 100.0) / total_movies.count) AS proportion FROM total_movies, movies_in_range;"
        ######################################################################
    ...
    def part_4(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_4_sql = "SELECT cast_name,COUNT(movie_id)FROM movie_cast WHERE popularity > 10 GROUP BY cast_name ORDER BY COUNT(movie_id) DESC,cast_name LIMIT 5"
        ######################################################################
    ...
    def part_5(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_5_sql = "SELECT title,printf('%.2f',score) AS movie_score,COUNT(cast_id) as cast_count from movies INNER JOIN movie_cast on movie_cast.movie_id=movies.id GROUP By movies.id ORDER BY score DESC, cast_count ASC,title ASC LIMIT 5"
        ######################################################################
    ...
    def part_6(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_6_sql = "select cast_id,cast_name As cast_name,printf('%.2f',avg(score)) FROM movie_cast INNER JOIN movies on movie_cast.movie_id=movies.id where movies.score>25 GROUP by cast_id HAVING COUNT(movie_id)>2 ORDER BY avg(score) DESC,cast_name ASC LIMIT 10"
        ######################################################################
    ...
    def part_7(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_7_sql = "CREATE VIEW good_collaboration AS SELECT a.cast_id AS cast_member_id1, b.cast_id AS cast_member_id2, COUNT(a.movie_id) AS movie_count, printf('%.2f', AVG(c.score)) AS average_movie_score FROM movie_cast a JOIN movie_cast b ON a.movie_id = b.movie_id JOIN movies c ON a.movie_id = c.id WHERE a.cast_id < b.cast_id GROUP BY a.cast_id, b.cast_id HAVING COUNT(a.movie_id) >= 2 AND AVG(c.score) >= 40;"
        ######################################################################
        return self.execute_query(connection, part_7_sql)
    
    def part_8(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_8_sql = "SELECT mc.cast_id, mc.cast_name, printf('%.2f', AVG(gc.average_movie_score)) AS collaboration_score FROM good_collaboration gc JOIN movie_cast mc ON gc.cast_member_id1 = mc.cast_id OR gc.cast_member_id2 = mc.cast_id GROUP BY mc.cast_id, mc.cast_name ORDER BY collaboration_score DESC, mc.cast_name LIMIT 5;"
        ######################################################################
    ...
    def part_9_a(self,connection,path):
        ############### EDIT SQL STATEMENT ###################################
        part_9_a_sql = "CREATE VIRTUAL TABLE movie_overview USING fts4(id INTEGER, overview TEXT)"
        ######################################################################
    ...
    def part_9_b(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_9_b_sql = "SELECT COUNT(id) as count_overview FROM movie_overview WHERE overview MATCH 'fight'"
        ######################################################################
    ...
    def part_9_c(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_9_c_sql = "SELECT COUNT(id) as movie_count FROM movie_overview WHERE overview MATCH 'space NEAR/5 program'"
        ######################################################################
