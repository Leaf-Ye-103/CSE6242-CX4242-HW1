...
class Graph:
    ...
    def add_edge(self, source: str, target: str) -> None:
        ...
        """
        e = (source, target)
        e_other = (target, source)
        if e not in self.edges and e_other not in self.edges:
            self.edges.append(e)
    ...
    def max_degree_nodes(self) -> dict:
        ...
        for e in self.edges:
            ...
            if e[1] not in deg:
                deg[e[1]] = 1#counting edges for the ending node
            else:
                deg[e[1]] += 1
        max_deg = 0            
        for v in deg.values():          
    ...
    def leaf_nodes(self) -> dict:
        deg = {}
        for e in self.edges:
            if e[0] not in deg: #counting edges for the starting node
                deg[e[0]] = 1
            else:
                deg[e[0]] += 1
            if e[1] not in deg: #counting edges for the ending node
                deg[e[1]] = 1
            else:
                deg[e[1]] += 1
        return deg
...
class  TMDBAPIUtils:
    ...
    def get_movie_cast(self, movie_id:str, limit:int=None, exclude_ids:list[int]=None) -> list:
        ...
        """
        extraURL = "/3/movie/"+movie_id+"/credits?api_key="+self.api_key+"&language=en-US"
        #print(extraURL)
        
        conn = http.client.HTTPSConnection("api.themoviedb.org")
        conn.request("GET", extraURL)
        r = conn.getresponse()
        movie_data_list = json.loads(r.read().decode('utf-8'))
        movie_cast = movie_data_list.get('cast', [])
        if exclude_ids:
            movie_cast = [c for c in movie_cast if c['id'] not in exclude_ids]
        if limit:
            movie_cast = [c for c in movie_cast if c['order'] < limit]
        return movie_cast
    ...
    def get_movie_credits_for_person(self, person_id: str, start_date: str = None, end_date: str = None) -> list:
        ...
        extraURL = "/3/person/"+person_id+"/movie_credits?api_key="+self.api_key+"&language=en-US"
        conn = http.client.HTTPSConnection("api.themoviedb.org")
        conn.request("GET", extraURL)
        r1 = conn.getresponse()
        #print(r1.status, r1.reason)
        credit_data = r1.read()
        credit_data_list = json.loads(credit_data.decode('utf-8'))
        # print(credit_data_list)
        
        credits = credit_data_list['cast']
        # print(credits)
        ...
        base_credit = []
        for c in credits:
            if c['release_date'] and start_date <= datetime.strptime(c['release_date'], "%Y-%m-%d").date() <= end_date:
                # print(c['release_date'])
                base_credit.append(c)
        
        # credits = [c for c in credits if start_date <= datetime.strptime(c['release_date'], "%Y-%m-%d").date() <= end_date]
        return base_credit
...
if __name__ == "__main__":
    ...
    tmdb_api_utils = TMDBAPIUtils(api_key='d7181791d022072c8e3813feb18b9de0')
    base_credits = tmdb_api_utils.get_movie_credits_for_person(person_id='2975', start_date = '1999-01-01', end_date = '1999-12-31')
    ...
    new_nodes_added = {}  # Track newly added nodes
    all_edges = set()  # Track all edges to ensure correct count

    # First loop: Build the base graph with Laurence Fishburne's co-actors
    for credit in base_credits:
        cast = tmdb_api_utils.get_movie_cast(movie_id=str(credit['id']), limit=5)
        actor_ids = set()

        for c in cast:
            actor_id = str(c['id'])
            actor_name = c['name'].replace(',', '')

            if actor_id not in graph.nodes:
                graph.add_node(id=actor_id, name=actor_name)
                new_nodes_added[actor_id] = actor_name
                if actor_id != '2975':
                    graph.add_edge(source='2975',target=actor_id)

            

    # Expanding the graph for 2 iterations
    for _ in range(2):

        current_nodes = list(new_nodes_added.keys())  # Ensure iteration over correct data
        new_nodes_added = {}  # Reset for next iteration

        for node_id in current_nodes:
            credits = tmdb_api_utils.get_movie_credits_for_person(
                person_id=node_id, start_date='1999-01-01', end_date='1999-12-31')
            ...
            for credit in credits:
                ...
                for c in cast:
                    co_actor_id = str(c['id'])
                    co_actor_name = c['name'].replace(',', '')

                    if co_actor_id not in graph.nodes:
                        graph.add_node(id=co_actor_id, name=co_actor_name)
                        new_nodes_added[co_actor_id] = co_actor_name
                    
                    if (node_id, co_actor_id) not in graph.edges and (co_actor_id,node_id) not in graph.edges and node_id!=co_actor_id:
